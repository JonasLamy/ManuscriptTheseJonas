...
\begin{align}
    ...
    \label{eq:test_1}
\end{align}
\newV{Cette erreur était accrue par le fait que notre algorithme de calcul des métriques est cumulatif seuil après seuil (Sec. \ref{sec:perf_bench}, Alg. \ref{alg:calcul_metriques}). La réécriture de ce test en incluant un epsilon permet de résoudre cette inconsistance (Eq. \ref{eq:test_2}).}
 \begin{align}
    p(x) &=
    \begin{cases}
    ...
    \label{eq:test_2}
\end{align}
\newV{avec $\epsilon = \frac{1}{ \text{Nb seuils} } 1.10^{-3}$. Dans notre banc de test, l'utilisateur choisit le nombre de seuils. La valeur de $\epsilon$ est donc définie de manière adaptative afin qu'elle soit toujours inférieure au pas entre deux seuils.}
