...
\newV{La nature de la représentation des nombres en flottant couplé à un seuillage binaire provoquait des inconsistances dans la répartition des voxels après le seuil. Par exemple, un test d'infériorité au sens large (Eq. \ref{eq:test_1}) appliqué au seuil $0.6$ classifie incorrectement un voxel flottant pour lequel $0.6$ est approximé par $0.59999999$}.
...
\newV{Cette erreur était accrue par le fait que notre algorithme de calcul des métriques est cumulatif seuil après seuil (Sec. \ref{sec:perf_bench}. \ref{alg:calcul_metriques}). La réécriture de ce test en incluant un epsilon permet de résoudre cette inconsistance (Eq. \ref{eq:test_2}).}
...
\newV{avec $\epsilon = \frac{1}{ \text{Nb seuils} } 1.10^{-3}$. Dans notre banc de test, l'utilisateur choisi le nombre de seuils. La valeur de $\epsilon$ est donc définie de manière adaptative afin qu'il soit toujours inférieur au pas entre deux seuils.}
